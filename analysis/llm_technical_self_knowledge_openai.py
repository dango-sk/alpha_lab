...
실행: python analysis/llm_technical_self_knowledge_openai.py [--date YYYY-MM-DD]
결과: analysis/results/llm_self_knowledge_openai_{stamp}.xlsx
"""
import os
import sys
import time
...
def _call_openai(system_prompt: str, user_msg: str) -> dict:
    """OpenAI API 호출 + 토큰/시간 측정."""
...
def run_technical_agent_openai(tech_data: dict) -> dict:
    """방식 A: 계산된 지표 텍스트 → OpenAI."""
    stock_summaries = []
...
- 캔들: 도지={ind.get('doji')}, 망치형={ind.get('hammer')}, 장악형={ind.get('bullish_engulfing')}"""
    ...
    return _call_openai(TECHNICAL_AGENT_PROMPT, user_msg)
...
def run_ohlcv_agent(conn, stocks: list[tuple[str, float]], calc_date: str) -> dict:
    ...
    return _call_openai(OHLCV_AGENT_PROMPT, user_msg)
...
def run_name_only_agent(conn, stocks: list[tuple[str, float]], calc_date: str) -> dict:
    ...
    return _call_openai(NAME_ONLY_AGENT_PROMPT, user_msg)
...
def run_comparison(calc_date: str = None):
    ...
    print(f"\n{'='*60}")
    print(f"  LLM 기술적 분석 자체지식 검증 (OpenAI)")
    print(f"  기준일: {calc_date}  모델: {OPENAI_MODEL}")
    print(f"{'='*60}")
    ...
    # ── 방식 A ──
    print("\n[2/5] 방식 A 실행 (계산된 지표 텍스트 → OpenAI)...")
    t0        = time.time()
    tech_data = collect_technical_data(conn, stocks, calc_date)
    result_a  = run_technical_agent_openai(tech_data)
    ...
    out_path = out_dir / f"llm_self_knowledge_openai_{stamp}.xlsx"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="LLM 기술적 분석 자체지식 검증 (OpenAI)")
