...
실행: python analysis/llm_technical_self_knowledge.py [--date YYYY-MM-DD]
결과: analysis/results/llm_self_knowledge_{stamp}.xlsx
"""
import sys
import time
import argparse
...
def run_ohlcv_agent(conn, stocks: list[tuple[str, float]], calc_date: str) -> dict:
    ...
    return _call_claude(OHLCV_AGENT_PROMPT, user_msg)
...
def run_name_only_agent(conn, stocks: list[tuple[str, float]], calc_date: str) -> dict:
    ...
    return _call_claude(NAME_ONLY_AGENT_PROMPT, user_msg)
...
def _call_claude(system_prompt: str, user_msg: str) -> dict:
    """Claude API 호출 + 토큰/시간 측정."""
...
def run_comparison(calc_date: str = None):
    ...
    print(f"\n{'='*60}")
    print(f"  LLM 기술적 분석 자체지식 검증")
    print(f"  기준일: {calc_date}  모델: {CLAUDE_MODEL}")
    print(f"{'='*60}")
    ...
    out_path = out_dir / f"llm_self_knowledge_{stamp}.xlsx"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="LLM 기술적 분석 자체지식 검증")
