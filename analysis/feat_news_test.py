"""
analysis/feat_news_test.py
뉴스 score(macro_tightening, macro_stress)를 HSMM feature로 추가 robustness 테스트.
base(breadth5+FX) vs +news(+T+S). Recovery 제외(후행성). 5 seed.
지표: Recall/Lead/FA/Whipsaw/격차(mean±std + consensus) + 2018·2022·2023 detection.
결측 score(pre-2018·gap): ffill 후 전체평균 fill(중립).
사용: DATABASE_URL=<ip> .venv/bin/python analysis/feat_news_test.py
"""
...
def build():
    ...
    # 뉴스 score: 월 인덱스에 맞춰 ffill + 평균 fill
    sc = pd.read_csv(A / "news_scores_2018_2026.csv").set_index('ym')
    tser = pd.Series(sc['tight']).reindex(yms).ffill(); sser = pd.Series(sc['stress']).reindex(yms).ffill()
    tser = tser.fillna(tser.mean()); sser = sser.fillna(sser.mean())
    TS = np.column_stack([tser.values, sser.values])
...
def main():
    BR, FX, TS, yms, n, ret, dd6 = build()
    base = np.hstack([BR, FX])
    CONFIGS = {"base(+FX)": base, "+News(T+S)": np.hstack([base, TS])}
    ...
    print("=== 이벤트 지표 (overlap, consensus + seed μ±std) ===")
    print(f"  {'구성':12}{'Recall':>9}{'Lead':>6}{'FA':>5}{'Whip':>6}{'격차(ov)':>18}")
    cons_maps = {}
    for name, X in CONFIGS.items():
        ...
        cons = ["Bear" if bsum[t] >= 3 else "Bull" for t in range(n)]; cons_maps[name] = cons
        rc, ne, ld, fa, wh, gp = ev(cons, ov, n, ret, dd6)
        gaps = [p[5] for p in per]
        print(f"  {name:12}{rc}/{ne:<6}{ld:>+5.1f}{fa:>5}{wh:>6}   {gp:>+5.2f}p (μ{np.mean(gaps):+.2f}±{np.std(gaps):.2f})", flush=True)

    def first_bear(c, lo, hi):
        return next((y for y in yms if lo <= y <= hi and c[yidx[y]] == "Bear"), "—")
    ...
    print(f"\n=== 긴축형 detection & 2023 recovery ===")
    print(f"  {'구성':12}{'2018첫Bear':>11}{'2018lead':>9}{'2022첫Bear':>11}{'2022lead':>9}{'2023Bear월':>10}{'2023막Bear':>11}")
    for name, c in cons_maps.items():
        ...
        print(f"  {name:12}{first_bear(c,'2018-01','2018-12'):>11}{f18:>9}{first_bear(c,'2021-12','2022-12'):>11}{f22:>9}{n23:>10}{last_bear(c,'2022-10','2023-12'):>11}")
    print("\n  판정: Recall/Lead/FA/Whip 유지 + 2022 첫Bear/lead 빨라지고 2023Bear월 안 늘면 → 보강 성공 → FCF.")
