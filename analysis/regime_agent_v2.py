"""
Trading Agent V2 - KOSPI 강세/약세 판정 (멀티모델 + 변화율 feature + 뉴스 이벤트화)
2023-11 ~ 현재까지 매월 실행 후 실제 KOSPI 수익률과 비교

V1 대비 변경:
- 변화율 feature 추가 (MA 기울기, RSI 변화, VIX 가속도, 수급 가속도)
- 뉴스 이벤트화 (카테고리/방향/강도/지속성)
- 멀티모델: Gemini(분석) + GPT-4o(반론) + Claude(최종)

실행: python analysis/regime_agent_v2.py
"""
import os
import sys
import json
...
TECH_SYSTEM = (
    ...
    "특히 [변화율 Feature] 섹션의 기울기/변화량에 주목하여 추세 전환 신호가 있는지 분석하세요. "
...
FUND_SYSTEM = (
    ...
    "특히 [매크로 변화율 Feature] 섹션의 변화 방향과 가속도에 주목하여 경기 전환 신호가 있는지 분석하세요. "
    f"{NO_EXTERNAL} {CONCISE} "
    "형식: 포인트1: ...\n포인트2: ...\n포인트3: ..."
)
BULL1_SYSTEM = (
...
def run_technical_agent(summary: str) -> str:
    """기술적 분석 → Gemini"""
    return call_agent(system_prompt=TECH_SYSTEM, user_content=summary, max_tokens=2048, model="gemini")


def run_fundamental_agent(summary: str) -> str:
    """매크로 분석 → Gemini"""
    return call_agent(system_prompt=FUND_SYSTEM, user_content=summary, max_tokens=2048, model="gemini")


# V2: 뉴스 이벤트화 프롬프트 (카테고리/방향/강도/지속성)
NEWS_SYSTEM = (
    "당신은 매크로 뉴스 이벤트 추출 전문가입니다.\n"
    "주어진 뉴스들에서 KOSPI에 영향을 미치는 핵심 이벤트만 추출하세요.\n"
    "개별종목 뉴스는 무시하고 매크로/시장 전체 흐름만 추출하세요.\n\n"
    "각 이벤트는 다음 형식으로 출력하세요:\n"
    "이벤트1: [카테고리] 설명 | 방향: +/- | 강도: 1~5 | 지속성: 단기/중기/장기\n\n"
    "카테고리: 금리, 경기, 환율, 수급, 지정학, 산업(반도체/AI 등)\n"
    "방향: + (KOSPI에 긍정), - (KOSPI에 부정)\n"
    "강도: 1(미미) ~ 5(매우 강함)\n"
    "지속성: 단기(1개월 이내), 중기(1~3개월), 장기(3개월 이상)\n\n"
    "최대 5개 이벤트만 추출하세요. Bull/Bear 판정은 하지 마세요.\n"
    f"{NO_EXTERNAL}"
)

def run_news_agent(as_of: date) -> tuple[str, str]:
    """뉴스 이벤트 추출 → Gemini. 뉴스 없으면 (None, None)"""
    news = get_news_summary(as_of)
    if not news:
        return None, None
    news_input = f"[{as_of.strftime('%Y-%m')} 직전월 매크로 뉴스]\n{news}"
    result = call_agent(system_prompt=NEWS_SYSTEM, user_content=news_input, max_tokens=2048, model="gemini")
    return result, news_input


def run_bull1(tech: str, fund: str) -> str:
    """1라운드: Bull 독립 주장 → Gemini (분석 모델과 동일)"""
    return call_agent(system_prompt=BULL1_SYSTEM,
                      user_content=f"[기술적 분석]\n{tech}\n\n[펀더멘털 분석]\n{fund}",
                      max_tokens=2048, model="gemini")


def run_bear1(tech: str, fund: str) -> str:
    """1라운드: Bear 독립 주장 → GPT-4o (다른 모델로 독립성 확보)"""
    return call_agent(system_prompt=BEAR1_SYSTEM,
                      user_content=f"[기술적 분석]\n{tech}\n\n[펀더멘털 분석]\n{fund}",
                      max_tokens=2048, model="gpt")


def run_bull2(tech: str, fund: str, bull1: str, bear1: str) -> str:
    """2라운드: Bull이 Bear 주장 보고 재반박 → Gemini"""
    return call_agent(
...
def run_manager_agent(manager_user: str) -> dict:
    ...
    for attempt in range(3):
        result = call_agent(system_prompt=MANAGER_SYSTEM, user_content=manager_user, max_tokens=2048, model="claude")
        try:
...
def run_month(as_of: date, past_results: list = None) -> dict:
    ...
    print("  [1/6] Technical Agent (Gemini)...")
    prompts['technical'] = {"system": TECH_SYSTEM, "user": tech_input, "model": "Gemini"}
    tech = run_technical_agent(tech_input)
    log_agent("Technical [Gemini]", TECH_SYSTEM, tech_input, tech)

    print("  [2/6] Fundamental Agent (Gemini)...")
    prompts['fundamental'] = {"system": FUND_SYSTEM, "user": fund_input, "model": "Gemini"}
    fund = run_fundamental_agent(fund_input)
    log_agent("Fundamental [Gemini]", FUND_SYSTEM, fund_input, fund)

    print("  [3/6] News Event Agent (Gemini)...")
    news_analysis, news_input = run_news_agent(as_of)
    if news_analysis:
        prompts['news'] = {"system": NEWS_SYSTEM, "user": news_input, "model": "Gemini"}
        log_agent("News Event [Gemini]", NEWS_SYSTEM, news_input, news_analysis)
    else:
        news_analysis = ""
        news_input = ""
    ...
    print("  [4/6] 1라운드: Bull(Gemini) / Bear(GPT-4o) 독립 주장...")
    prompts['bull1'] = {"system": BULL1_SYSTEM, "user": bull1_user, "model": "Gemini"}
    bull1 = call_agent(system_prompt=BULL1_SYSTEM, user_content=bull1_user, max_tokens=2048, model="gemini")
    log_agent("강세론자 1R [Gemini]", BULL1_SYSTEM, bull1_user, bull1)
    prompts['bear1'] = {"system": BEAR1_SYSTEM, "user": bear1_user, "model": "GPT-4o"}
    bear1 = call_agent(system_prompt=BEAR1_SYSTEM, user_content=bear1_user, max_tokens=2048, model="gpt")
    log_agent("약세론자 1R [GPT-4o]", BEAR1_SYSTEM, bear1_user, bear1)

    bull2_user = f"[내 주장 (1라운드)]\n{bull1}\n\n[상대방 Bear 주장]\n{bear1}\n\n[참고 데이터]\n기술적: {tech[:300]}\n펀더멘털: {fund[:300]}"
    bear2_user = f"[내 주장 (1라운드)]\n{bear1}\n\n[상대방 Bull 주장]\n{bull1}\n\n[참고 데이터]\n기술적: {tech[:300]}\n펀더멘털: {fund[:300]}"

    print("  [5/6] 2라운드: Bull(Gemini) / Bear(GPT-4o) 재반박...")
    prompts['bull2'] = {"system": BULL2_SYSTEM, "user": bull2_user, "model": "Gemini"}
    bull2 = call_agent(system_prompt=BULL2_SYSTEM, user_content=bull2_user, max_tokens=1024, model="gemini")
    log_agent("강세론자 2R [Gemini]", BULL2_SYSTEM, bull2_user, bull2)
    prompts['bear2'] = {"system": BEAR2_SYSTEM, "user": bear2_user, "model": "GPT-4o"}
    bear2 = call_agent(system_prompt=BEAR2_SYSTEM, user_content=bear2_user, max_tokens=1024, model="gpt")
    log_agent("약세론자 2R [GPT-4o]", BEAR2_SYSTEM, bear2_user, bear2)
...
def main():
    # 전체 백테스트 (V1과 동일 범위)
    test_months = [date(2018 + (3 + i) // 12, (3 + i) % 12 + 1, 1) for i in range(97)]

    # V2 결과 파일 (V1과 분리)
    out_path = Path(__file__).parent / "regime_agent_v2_results.json"
    existing = []
    if out_path.exists():
    ...
    # HTML 리포트 생성
    html_path = Path(__file__).parent / "regime_agent_v2_report.html"
