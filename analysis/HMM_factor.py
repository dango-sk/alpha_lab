...
def garman_klass_volatility(high, low, close, open):
    return np.sqrt(0.5 * np.log(high / low)**2 - (2 * np.log(2) - 1) * np.log(close / open)**2)

df['gk_vol_GSPC'] = garman_klass_volatility(df['High_GSPC'], df['Low_GSPC'], df['Close_GSPC'], df['Open_GSPC'])
df['realized_vol_GSPC'] = df['gk_vol_GSPC'].rolling(window=21).mean() * np.sqrt(252)
df.dropna(inplace=True)

# 월별 리샘플링
agg_rules = {
    'Open_GSPC': 'first', 'High_GSPC': 'max', 'Low_GSPC': 'min', 'Close_GSPC': 'last', 'realized_vol_GSPC': 'last',
    'Open_VIX': 'first', 'High_VIX': 'max', 'Low_VIX': 'min', 'Close_VIX': 'last'
}
monthly_df = df.resample('M').agg(agg_rules)
final_column_names = {
    'Open_GSPC': 'sp500_Open', 'High_GSPC': 'sp500_High', 'Low_GSPC': 'sp500_Low', 'Close_GSPC': 'sp500_Close', 'realized_vol_GSPC': 'sp500_realized_vol',
...
# 전체 기간 S&P 500 지수 플롯
ax.plot(features.index, features['sp500_Close'], label='S&P 500 종가', color='black', linewidth=1.5)
...
ax.set_title('HMM 시장 국면 분석 (Walk-Forward Out-of-Sample 검증 결과)', fontsize=20)
ax.set_xlabel('날짜', fontsize=15)
ax.set_ylabel('S&P 500 지수', fontsize=15)
